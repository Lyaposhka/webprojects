...
class Category(Base):
    __tablename__ = 'categories'                                                       #Категории
...
class User(Base, UserMixin):
    ...
    middle_name: Mapped[Optional[str]] = mapped_column(String(100))                         #Имя пользователя
...
class Course(Base):
    ...
    background_image_id: Mapped[str] = mapped_column(ForeignKey("images.id"))                   #Курсы
    created_at: Mapped[datetime] = mapped_column(default=datetime.now)
...
class Image(db.Model):
    ...
    md5_hash: Mapped[str] = mapped_column(String(100), unique=True)          #Картинки
...
class Review(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)                                #Класс отзывов
